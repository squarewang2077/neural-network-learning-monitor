...
def main():
    ...
    parser.add_argument('--act_name', type=str, default='relu', 
                        help='the activation')
    parser.add_argument('--num_hidden_layers', type=int, default=7, # should be 1, 3, 7 
    ...
    model = DeepMLP(input_size=3*32*32, hidden_sizes=[256]*args.num_hidden_layers, output_size=10, activation=args.act_name)
    ...
    result_eval_df.to_csv(f'./log/results/cross_layer_eval_MLP(256x{args.num_hidden_layers + 1}).csv', index=False)
    result_mean_tau_df.to_csv(f'./log/results/cross_layer_mean_tau_MLP(256x{args.num_hidden_layers + 1}).csv', index=False)
    result_std_tau_df.to_csv(f'./log/results/cross_layer_std_tau_MLP(256x{args.num_hidden_layers + 1}).csv', index=False)
